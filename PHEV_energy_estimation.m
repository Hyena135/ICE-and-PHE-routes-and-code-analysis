...
for k = 1:n
    ...
    regen_mask = a_seg <= thr_regen_mps2;
    F_iner_neg = abs(min(F_iner, 0));
    E_rec_J = eta_regen * (F_iner_neg .* dist_seg_m) .* regen_mask;
    E_J = F_pos .* dist_seg_m - E_rec_J;
...
for k = 1:n
    ...
    yyaxis left
    h1 = plot(t_list{k}, v_list{k}, 'k-', 'LineWidth', 1.2);
    ...
    yyaxis right
    h2 = plot(t_list{k}, Eacum_list{k}, 'r-', 'LineWidth', 1.2);
...
set(gcf,'Color','w');
exportgraphics(gcf,'CiclosVelocidad_Energia.pdf','ContentType','vector');
disp('Gráfico exportado como "CiclosVelocidad_Energia.pdf"');
